...
class DecisionTreeService
{
     ...
     */
    public function buildTree($data, $labelField = 'Play')
    {
        $attributes = $this->calculationService->getAttributes($data);
        return $this->buildTreeRecursive($data, $attributes, $labelField);
    }

    /**
     * Recursive function untuk build tree
     */
    protected function buildTreeRecursive($data, $attributes, $labelField = 'Play', $depth = 0)
    {
        ...
        foreach ($attributes as $attr) {
            $gain = $this->calculationService->calculateGain($data, $attr, $labelField);
            if ($gain > $maxGain) {
                $maxGain = $gain;
                $bestAttribute = $attr;
            }
        ...
        foreach ($subsets as $value => $subset) {
            $children[$value] = $this->buildTreeRecursive($subset, $remainingAttrs, $labelField, $depth + 1);
        }
